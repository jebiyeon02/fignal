...
test("베타 안내 닫기 상태를 hover 스타일이 덮어쓰지 않는다", () => {
  assert.match(rail, /onClick=\{\(\) => setBetaOpen\(false\)\}/);
  assert.match(styles, /\.pinned \.betaPanel \{[\s\S]*pointer-events: auto;[\s\S]*visibility: visible;/);
  assert.doesNotMatch(styles, /\.betaWidget:hover \.betaPanel/);
});
